...
def setup(request):     #setup function is defined and
                        # request is predefined object, which will be auto populated by pytest
    global driver
    browser_name=request.config.getoption("browser_name")   #pytest --browser_name "firefox"
    # we can insert any name in bracket like already inserted("browser_name")
